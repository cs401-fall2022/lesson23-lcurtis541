      ...
      db.all(`SELECT name FROM sqlite_master WHERE type='table' AND name='blog'`,
        (err, rows) => {
          if (rows.length === 1) {
            ...
            db.all(` select blog_title, blog_txt from blog`, (err, rows) => {
            ...
            db.exec(`create table blog (
                     ...
                     blog_txt text NOT NULL);
                      ...
                      values ('Text Title','Text Body'),
                             ('Oh my goodness blogging is fun','Text the body here');`,
              () => {
                db.all(` select blog_title, blog_txt from blog`, (err, rows) => {
                  res.render('index', { title: 'Express', data: rows });
                });
  ...
  var db = new sqlite3.Database('mydb.sqlite3',
    ...
    (err) => {
      ...
      db.exec(`insert into blog (blog_title,blog_txt)
                values ('${req.body.blog_title}','${req.body.blog_txt}');`)
...
router.post('/edit', (req, res, next) => {
  ...
  var db = new sqlite3.Database('mydb.sqlite3',
    ...
    (err) => {
      ...
      //HINT: the answer is in the XKCD comic on the home page little bobby tables :)
      db.exec(`UPDATE blog
              SET blog_title='${req.body.blog_title}',blog_txt='${req.body.blog_txt}'
              WHERE blog_title='${req.body.old_blog_title}';`)
      //redirect to homepage
      res.redirect('/');
    }
  );
...
router.post('/delete', (req, res, next) => {
  ...
  var db = new sqlite3.Database('mydb.sqlite3',
    ...
    (err) => {
      ...
      //Try and figure out how why this is unsafe and how to fix it.
      //HINT: the answer is in the XKCD comic on the home page little bobby tables :)
      db.exec(`DELETE FROM blog WHERE blog_title='${req.body.blog_title}';`);     
